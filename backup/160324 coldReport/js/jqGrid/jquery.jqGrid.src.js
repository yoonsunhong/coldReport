...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		setPager = function (pgid, tp){
			...
			if(dir=="rtl") { pgl += str; }
			if (ts.p.pginput === true) { pginp = "<td dir='" + dir + "'>" + $.jgrid.format(ts.p.pgtext || "", "<input class='ui-pg-input' id='goPage" + $.jgrid.jqID(pgid) + "' type='text' size='2' style='width:40px;text-align:center;' maxlength='7' value='0' role='textbox' onkeypress='return event.charCode >= 48 && event.charCode <= 57'/>", "<span id='sp_1_" + $.jgrid.jqID(pgid) + "'></span>") + " </td><td class='ui-pg-button ui-corner-all'><span onclick='goPage(" + pgid + ")' >이동</span></td>"; }
			if(ts.p.pgbuttons===true) {
				var po=["first"+tp,"prev"+tp, "next"+tp,"last"+tp]; if(dir=="rtl") { po.reverse(); }
				pgl += "<td id='"+po[0]+"' class='ui-pg-button ui-corner-all'><span class=''> 처음 </span></td>";
				pgl += "<td id='"+po[1]+"' class='ui-pg-button ui-corner-all'><span class=''> 이전 </span></td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";
				pgl += "<td id='"+po[2]+"' class='ui-pg-button ui-corner-all'><span class=''> 다음 </span></td>";
				pgl += "<td id='"+po[3]+"' class='ui-pg-button ui-corner-all'><span class=''> 끝 </span></td>";
			...
			$('input.ui-pg-input',"#"+pgcnt).keypress( function(e) {
				...
				if(key == 13) {
					...
					if(flag == 'prowed1'){
						...
						maxPage1 = $('#sp_1_prowed1').text();
						movePage1 = $('#goPageprowed1').val();
					...
					}else if(flag == 'prowed2'){
						maxPage2 = $('#sp_1_prowed2').text();
						movePage2 = $('#goPageprowed2').val();
						
						if(Number(movePage2) > Number(maxPage2)){
							alert("총 페이지 수보다 큽니다. 다시 입력 해주세요.");
							return;
						}else{
							$('#grid2').jqGrid('setGridParam', { "page": movePage2 }).trigger("reloadGrid");
						}
					}else if(flag == 'prowed3'){
						maxPage3 = $('#sp_1_prowed3').text();
						movePage3 = $('#goPageprowed3').val();
						
						if(Number(movePage3) > Number(maxPage3)){
							alert("총 페이지 수보다 큽니다. 다시 입력 해주세요.");
							return;
						}else{
							$('#grid3').jqGrid('setGridParam', { "page": movePage3 }).trigger("reloadGrid");
						}
					}else if(flag == 'prowed4'){
						maxPage4 = $('#sp_1_prowed4').text();
						movePage4 = $('#goPageprowed4').val();
						
						if(Number(movePage4) > Number(maxPage4)){
							alert("총 페이지 수보다 큽니다. 다시 입력 해주세요.");
							return;
						}else{
							$('#grid4').jqGrid('setGridParam', { "page": movePage4 }).trigger("reloadGrid");
						}
					}else{
						return;
					}
